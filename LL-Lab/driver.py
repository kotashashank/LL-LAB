...
def create_signals(signals_array, time_array, input_amount, graph_height, graph_width, x0, y0, fillIn, fillOut, width, step):
    ...
    for i in range(0, len(signals_array)):
        ...
        canvas.create_text(50, current_y + 10, text=i, fill='black', font='Helvetica 20')
    ...
    for t in range(0, len(time_array) - 1):
        ...
        for i in range(0, len(signals_array)):
            ...
            if(i >= input_amount):
                if(t + 1 < len(time_array) and not signals_array[i][t + 1]):
                ...
                if(t + 2< len(time_array) and signals_array[i][t + 1] != signals_array[i][t + 2]):
                    if(not signals_array[i][t + 1]):
        ...
        canvas.create_rectangle(graph_height + x0 + 152, y0 + graph_height - 50, graph_height +x0 + 1000, y0 + graph_height + 50, fill='white')
        canvas.create_text(graph_width + x0 + 50, y0 + graph_height, text=time_array[len(time_array) - 1], fill='grey', font="Helvetica 50 bold")
...
create_signals(c_output[0], c_output[1], c_output[2], 650, 800, 100, 100, fillIn='green', fillOut='red', width=4, step=0.03)
